...
template <unsigned int M, unsigned int N>
inline uint32_t gradientARGB(uint32_t pixFront, uint32_t pixBack) // find intermediate color between
// two colors with alpha channels
// (=> NO alpha blending!!!)
{
    static_assert(0 < M && M < N && N <= 1000, "");
    ...
    auto calcColor = [=](unsigned char colFront, unsigned char colBack) {
        return static_cast<unsigned char>((colFront * weightFront + colBack * weightBack) /
                                          weightSum);
    };
...
//{
// __asm //speeds up xBRZ by about 9% compared to std::sqrt which internally uses the same
// assembler instructions but adds some "fluff"
// {
// fld n
// fsqrt
// }
...
struct MatrixRotation {
    static const size_t I_old =
        N - 1 - MatrixRotation<static_cast<RotationDegree>(rotDeg - 1), I, J,
...
inline double distYCbCr(uint32_t pix1, uint32_t pix2, double lumaWeight) {
    ...
    // YCbCr conversion is a matrix multiplication => take advantage of linearity by subtracting
    // first!
    const int r_diff = static_cast<int>(getRed(pix1)) -
                       getRed(pix2); // we may delay division by 255 to after matrix multiplication
    const int g_diff = static_cast<int>(getGreen(pix1)) - getGreen(pix2); //
    const int b_diff = static_cast<int>(getBlue(pix1)) -
                       getBlue(pix2); // substraction for int is noticeable faster than for double!
...
private:
    ...
    double distImpl(uint32_t pix1, uint32_t pix2) const {
        ...
        return buffer[(((r_diff + 255) / 2) << 16) | // slightly reduce precision (division by 2) to
                      // squeeze value into single byte
                      (((g_diff + 255) / 2) << 8) | ((b_diff + 255) / 2)];
    }

    std::vector<float>
        buffer; // consumes 64 MB memory; using double is only 2% faster, but takes 128 MB
};
                      ...
                      const xbrz::ScalerCfg& cfg) // result: F, G, J, K corners of "GradientType"
{
    ...
    double jg = dist(ker.i, ker.f) + dist(ker.f, ker.c) + dist(ker.n, ker.k) + dist(ker.k, ker.h) +
                weight * dist(ker.j, ker.g);
    double fk = dist(ker.e, ker.j) + dist(ker.j, ker.o) + dist(ker.b, ker.g) + dist(ker.g, ker.l) +
                weight * dist(ker.f, ker.k);

    if (jg < fk) // test sample: 70% of values max(jg, fk) / min(jg, fk) are between 1.1 and 3.7
    // with median being 1.8
    {
...
// we cannot and NEED NOT write "ker.##x" since ## concatenates preprocessor tokens but "." is not a
// token
DEF_GETTER(a)
DEF_GETTER(b)
DEF_GETTER(c)
DEF_GETTER(d)
DEF_GETTER(e) DEF_GETTER(f) DEF_GETTER(g) DEF_GETTER(h) DEF_GETTER(i)
#undef DEF_GETTER
...
return ker.y;                                                                                      \
    ...
    DEF_GETTER(a, g) DEF_GETTER(b, d) DEF_GETTER(c, a) DEF_GETTER(d, h) DEF_GETTER(e, e)
        DEF_GETTER(f, b) DEF_GETTER(g, i) DEF_GETTER(h, f) DEF_GETTER(i, c)
#undef DEF_GETTER
...
return ker.y;                                                                                      \
    \
            ...
            DEF_GETTER(a, i) DEF_GETTER(b, h) DEF_GETTER(c, g) DEF_GETTER(d, f) DEF_GETTER(e, e)
                DEF_GETTER(f, d) DEF_GETTER(g, c) DEF_GETTER(h, b) DEF_GETTER(i, a)
#undef DEF_GETTER
...
return ker.y;                                                                                      \
    \
                    ...
                    DEF_GETTER(a, c) DEF_GETTER(b, f) DEF_GETTER(c, i) DEF_GETTER(d, b)
                        DEF_GETTER(e, e) DEF_GETTER(f, h) DEF_GETTER(g, a) DEF_GETTER(h, d)
                            DEF_GETTER(i, g)
#undef DEF_GETTER
...
#define i get_i<rotDeg>(ker)
    ...
    if (getBottomR(blend) >= BLEND_NORMAL) {
        ...
        const bool doLineBlend = [&]() -> bool {
            ...
            // make sure there is no second blending in an adjacent rotation for this pixel: handles
            // insular pixels, mario eyes
        ...
        if (doLineBlend) {
            const double fg = dist(f, g); // test sample: 70% of values max(fg, hc) / min(fg, hc)
...
template <class Scaler, class ColorDistance> // scaler policy: see "Scaler2x" reference
// implementation
void scaleImage(const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight,
    ...
    //"use" space at the end of the image as temporary buffer for "on the fly preprocessing": we
    // even could use larger area of
    //"sizeof(uint32_t) * srcWidth * (yLast - yFirst)" bytes without risk of accidental overwriting
    // before accessing
    ...
    // initialize preprocessing buffer for first row of current stripe: detect upper left and right
    // corner blending
    // this cannot be optimized for adjacent processing stripes; we must not allow for a memory race
    // condition!
    if (yFirst > 0) {
    ...
    for (int y = yFirst; y < yLast; ++y) {
        ...
        for (int x = 0; x < srcWidth; ++x, out += Scaler::scale) {
            ...
            const int x_m1 =
                std::max(x - 1, 0); // perf: prefer array indexing to additional pointers!
            const int x_p1 = std::min(x + 1, srcWidth - 1);
            const int x_p2 = std::min(x + 2, srcWidth - 1);
            ...
            unsigned char blend_xy = 0; // for current (x, y) position
            {
                ...
                */
                ...
                setBottomR(blend_xy, res.blend_f); // all four corners of (x, y) have been
                // determined at this point due to processing
                // sequence!

                setTopR(blend_xy1, res.blend_j); // set 2nd known corner for (x, y + 1)
                preProcBuffer[x] = blend_xy1;    // store on current buffer position for use on next
                // row

                blend_xy1 = 0;
                setTopL(blend_xy1, res.blend_k); // set 1st known corner for (x + 1, y + 1) and
            ...
            fillBlock(out, trgWidth * sizeof(uint32_t), ker4.f,
                      Scaler::scale); // place *after* preprocessing step, to not overwrite the
...
struct Scaler3x : public ColorGradient {
    ...
    static void blendCorner(uint32_t col, OutputMatrix& out) {
        ...
        // alphaGrad<7, 256>(out.template ref<2, 1>(), col); //0.02826017254 -> negligible + avoid
        // conflicts with other rotations for this odd scale
...
struct Scaler5x : public ColorGradient {
    ...
    static void blendCorner(uint32_t col, OutputMatrix& out) {
        ...
        // alphaGrad<1, 64>(out.template ref<4, 2>(), col); //0.01676812367 -> negligible + avoid
        // conflicts with other rotations for this odd scale
...
struct ColorDistanceARGB {
    static double dist(uint32_t pix1, uint32_t pix2, double luminanceWeight) {
        ...
        */
        ...
        // alternative? return std::sqrt(a1 * a2 * square(DistYCbCrBuffer::dist(pix1, pix2)) +
        // square(255 * (a1 - a2)));
    }
};
...
void xbrz::scale(size_t factor, const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight,
...
void xbrz::nearestNeighborScale(const uint32_t* src, int srcWidth, int srcHeight, int srcPitch,
    ...
    case NN_SCALE_SLICE_SOURCE:
        // nearest-neighbor (going over source image - fast for upscaling, since source is read only
        // once
        yFirst = std::max(yFirst, 0);
        yLast = std::min(yLast, srcHeight);
        ...
        for (int y = yFirst; y < yLast; ++y) {
            ...
            const int yTrg_first =
                (y * trgHeight + srcHeight - 1) / srcHeight; //=ceil(y * trgHeight / srcHeight)
            const int yTrg_last = ((y + 1) * trgHeight + srcHeight - 1) /
    ...
    case NN_SCALE_SLICE_TARGET:
        // nearest-neighbor (going over target image - slow for upscaling, since source is read
        // multiple times missing out on cache! Fast for similar image sizes!)
        yFirst = std::max(yFirst, 0);
